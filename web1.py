...
if __name__ == "__main__":
    ...
    server.serve_forever()        return None
...
<style>
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  :root {
    ...
    --fb: 'Courier New', Courier, monospace;
  }
  ...
  body::before {
    ...
    background-image:
      linear-gradient(rgba(110,76,255,.08) 1px, transparent 1px),
      linear-gradient(90deg, rgba(110,76,255,.08) 1px, transparent 1px);
  ...
  body::after {
    ...
    background: radial-gradient(ellipse, rgba(110,76,255,.22) 0%, transparent 70%);
    pointer-events: none;
  }
  ...
  .card {
    ...
    position: relative;
    z-index: 1;
    box-shadow: 0 0 60px rgba(110,76,255,.2), 0 24px 48px rgba(0,0,0,.7);
  ...
  input:focus {
    ...
    box-shadow: 0 0 0 3px rgba(110,76,255,.2);
  }
  ...
  .btn:hover {
    ...
    box-shadow: 0 8px 24px rgba(200,255,0,.35);
...
<div class="card">
  ...
  <form method="POST" action="/login">
    <div class="field">
      <label>Usuario</label>
      <input type="text" name="username" placeholder="seu nome" autocomplete="off" required>
    </div>
    <div class="field">
      <label>Senha</label>
...
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>YNot&amp;S &mdash; Notas</title>
<style>
  *, *::before, *::after {{ box-sizing: border-box; margin: 0; padding: 0; }}
  ...
  body::before {{
    ...
    position: fixed; inset: 0;
    background-image:
      linear-gradient(rgba(110,76,255,.05) 1px, transparent 1px),
      linear-gradient(90deg, rgba(110,76,255,.05) 1px, transparent 1px);
    background-size: 40px 40px;
    pointer-events: none;
    z-index: 0;
  }}
  ...
  .new-note {{
    ...
    padding: 22px;
    margin-bottom: 30px;
    box-shadow: 0 0 40px rgba(110,76,255,.08);
  }}
  ...
  .note-title-input:focus {{
    border-color: var(--accent2);
    box-shadow: 0 0 0 3px rgba(110,76,255,.18);
  }}
  ...
  .btn-add:hover {{
    transform: translateY(-2px);
    box-shadow: 0 8px 20px rgba(200,255,0,.28);
  }}
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path in ("/", "/login"):
            ...
            notas = get_notas()
            self.send_html(build_notes_page(notas))
        else:
            self.redirect("/")
    ...
    def do_POST(self):
        ...
        if self.path == "/login":
            ...
            if user == USER_NAME and pwd == USER_PASS:
                ...
                self.send_html(LOGIN_PAGE.replace("{ERROR}", ERROR_BLOCK))
            ...
            if titulo or conteudo:
                ...
                msg = ""
            notas = get_notas()
            self.send_html(build_notes_page(notas, msg))
            ...
            if nota_id:
                delete_nota(nota_id)
            notas = get_notas()
            self.send_html(build_notes_page(notas, "Nota apagada."))

        else:
            self.redirect("/")

# ── Inicialização — porta vinda do Render via $PORT ──────────

if __name__ == "__main__":
    port = int(os.environ.get("PORT", 8080))
    server = HTTPServer(("0.0.0.0", port), Handler)
    print(f"YNot&S rodando na porta {port}...")
